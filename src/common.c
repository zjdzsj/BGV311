...
*  All rights reserved 
*
*  FileName?锟??o   	common.c
*  Author?锟??o     	Roger
*  Date?锟??o  			2010-3-8 20:11
*  Revised Date?锟??o  	
*  Main Function?锟??o  this file contains all the functions may be used in all project files
*  Version:			BonaV2.0
*  Note?锟??o 			 			
*********************************************************************************************/
#pragma sfr
...
//断相事件记录处理 11
	...
	static uint32 s_lossPhaseCnt[3];		//断相判断时间 11

#endif

//过载事件记录 11
#if(OVER_POWER_EANBLED==1)	
	static uint32 s_overPowerCnt;		//过载事件时间 11
#endif
...
{   
	//?鈥??鈥??????????鈥??|???路?3?渭???????藟???a?????????鈥??渭 11
    PIOR = 0x10U;   // 管脚功能重新定向 11
    CRC0CTL = 0x00U;// 闪存CRC 控制寄存器 11
    RPECTL = 0x00U; // RAM 奇偶校验错误控制寄存器 11
    IAWCTL = 0x00U; // 非法存储器存取检测控制寄存 11
    PMS = 0x00U;    // 端口模式选择寄存器 11
    
	
	//CPU???卤?鈥??鈥??鈥????鈥??卤?鈥??篓?鈥??? 11
	InitMemAndClock (  );
	
	//?渭?鈥??渭?搂?鈥??|?鈧??---?????2???渭?搂?鈥??鈥????2???锟??2???????2?锟? 11
	InitEXLVI ( );
...
static void InitMemAndClock ( void )
{	
	// ?鈥????渭?搂?o?3???渭???3???1?鈥????????????????2?????卤?渭?鈥????路???卤?鈥??鈥??鈥??????? 11
    // ???卤?鈥??鈥??3?渭?????鈥??篓?鈥??????1?鈥????鈥????????鈥??鈥??????鈥??篓?鈥????锟????2??????option.asm 11

    // ???鈥??鈥??鈥????-???鈥??鈥??o?????????????????????渭???鈥??篓?鈥??? 11

    // ?鈥??????1?鈥????????2?????????????卤?渭?鈥????路???卤?鈥??鈥? 11 
   
    // Set fMX 
    // ???卤?鈥??鈥??鈥??????????锟????? 11
    // CMC???鈥??????o?3?鈥??????????鈥??鈥????鈥??? 11
    CMC = 0;        // ?2?????1?鈥???X1?o??XT1???卤?鈥??鈥? 11

    MSTOP = 1U;     // ???锟??鈥??1X1 11

    // Set fMAIN 
    MCM0 = 0U;      // ?鈥????鈥??卤?????????????2?????卤?渭?鈥????路???卤?鈥??鈥? 11

    // Set fSUB
    XTSTOP = 1U;    // ???锟??鈥??1XT1 11
    OSMC = (1<<4);  // RTC,IT,LCD?鈥????鈥??卤?????2???渭?????????卤?鈥??鈥? 15khZ 11

    // Set fCLK 4MHz
    CSS = 0U;       // CPU???卤?鈥??鈥??鈥????鈥??卤?鈥??路???渭???3???卤?鈥??鈥? 11
    
    // Set fIH 
    HIOSTOP = 0U;   // ?????????????2?????卤?渭?鈥????路?鈥??????? 11
...
static void InitEXLVI ( void )
{
	//LVD?3?渭???????藟?锟????鈥???????LVIF?卤?a???????鈥????鈥??锟???11 
	LVIM &= 0x7f;		//?????卤???鈥???????11
	LVIS = 0x01;
	LVIMK = 0;		//???1?????鈥??????? 11
...
{   
	...
	//全部使用数字接口 11
	PMC1 = 0xF0;
	PMC2 = 0x3f;	//P2.2 模拟口 11
	PMC4 = 0x00;

	
	//P2.0 P2.1 ad口选择 数字口 11
	ADPC = 0x01;
	
	//LCD管脚 P12.5 P12.6做为LCD引脚 11
	ISCLCD = 0x00;	
...
//            #endif            
		...
		(CheckDay31(g_date.day)==0) )
	{
				...
				(uchar*)&g_buf[0],CHECK_ADDMOD,16 ) == 0 )
		{
			if ( (g_date.year>=g_buf[5]) &&	
					...
					(g_date.day>=g_buf[7]) )		//一个阶梯周期完成 11
			{
				g_meterFlag.Bit.LadderProcess = 1; 
 				g_energy.CurrentQ = 0;	
 				
 				if ( (g_buf[0]>0) && (g_buf[0]<5) )		//年 11
 				{
 				...
 				else if ( (g_buf[1]>0) && (g_buf[1]<10) )	//月 11
 				{
	...
	//负载越限 11
#if(OVER_POWER_EANBLED==1)	
	#if(SINGLE_OR_THREE_METER==3)		//三相 11
...
//恶性负载使能 11
#if(MALIGNANT_LOAD_CHK_EANBLED==1) 
	#if(SINGLE_OR_THREE_METER==1)		//单相 11
	{
		if ( (g_buyPara.FactorLimit<1000) &&
				(g_buyPara.FactorLimit>500) &&		//0.500 3位小数 11
...
void InitGlobalVariable ( void )
{
	...
	if ( (g_para.MeterTpye&BIT1) == BIT1 )		//远程拉闸 11
...
{          
	...
	//处于命令拉闸状态 11
	else if ( (g_para.MeterTpye&BIT1) == BIT1 )		//远程拉闸 11
	{
		if ( g_relayChkDly == 0 )
	    {
    	    if ( g_relayFlag.Bit.RelayOn==1 )  //如果是合闸状态需要跳闸 11
                g_relayFlag.Bit.RelayNeedToOperate = 1;
            else
                g_relayFlag.Bit.RelayNeedToOperate = 0;	
        }		
			    ...
			    {	    		    		    		
			    	if ( g_relayFlag.Bit.RelayAlarmHappened==0 )
			    	{
				    	if ( g_energy.Balance <= (((uint32)g_buyPara.Alarm2)*10000) )
				        {
				            ...
				            //因用台体走字，拉闸后掉电太快 丢数据 2016/01/06  11
...
{          
    ...
    if( g_relayFlag.Bit.RelayOn )   //relay off  11
    {
    	...
    	if ( (g_para.MeterTpye&BIT5) == BIT5 )		//电平式 11
    	{
    		g_relayCnt = 0;			
    	}
    	else
...
{	
	...
	for(cmp_num=10;cmp_num>0;cmp_num--)
	{
		if ( LVIF )
		{
			for(i=0;i<200;i++);		//???搂?1???渭?鈥??渭?搂?鈥??2?????????????2?锟?10?鈥??? 11			
		}
		else
			break;					//?????渭?鈥??渭?搂?鈥??2?路?渭???? 11
	}
    //EI();
	if(cmp_num==0)
		return(1);
